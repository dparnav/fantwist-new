...
//Create MLB Projections and Contests

function create_mlb_projections_and_contests2($date, $projection_key) {
	...
	$projections_url = "https://fly.sportsdata.io/v3/mlb/projections/JSON/PlayerGameProjectionStatsByDate/$date";
...
function update_mlb_projection_scores($projection_key) {
	...
	foreach($update_query->posts as $updatepost){
		echo $current_contest_date = get_field('contest_date',$updatepost->ID);

		if($current_contest_date >= current_time( 'timestamp')){
			echo$date = date('Y-M-d',$current_contest_date);
...
//New Update Function
function update_mlb_live_scores($stats_key) {
...
function update_projection_for_mlb_contest($stats_key) {
	...
	if ( $the_query->have_posts() ) {
		...
		while ( $the_query->have_posts() ) {
			...
			$response = wp_remote_get( "https://fly.sportsdata.io/v3/mlb/projections/json/PlayerGameProjectionStatsByDate/$contest_date_url", array(
				'method'	=> 'GET',
				'timeout'	=> 60,
			    'headers'	=> array(
			        'Ocp-Apim-Subscription-Key' => $stats_key,
