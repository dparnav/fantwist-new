...
if (isset($_POST['update_live_nfl_contests_button_clicked'])){
	...
	$stats_key = 'f75cd5d61287488292aa33b77a898aa6';//'cc1a0c813b9b4267990116eb45c900b9';
...
//demo contest
if (isset($_POST['create_nfl_demo_projections_and_contests_button_clicked'])) {
	
	if (isset($_POST['number-of-plays'])) {
		$numofplays = $_POST['number-of-plays'];
	}
	else {
		$numofplays = 0;
	}
	
	$projection_key = 'f75cd5d61287488292aa33b77a898aa6';

	create_nfl_demo_projections_and_contests($numofplays,$projection_key);
...
'</div>' .
'<div class="table-row">' . '<div class="table-cell"><input type="radio" name="contest-type" value="regular" checked>Regular <input type="radio" name="contest-type" value="demo">Demo</div>' .
'<div class="table-cell league-tasks"><ul style="list-style: disc; padding-left: 20px;"><li class="regular"><form action="options-general.php?page=pariwager-processing" method="post" style="margin:0;">';	
...
echo '</form><hr></li><li class="regular"><form action="options-general.php?page=pariwager-processing" method="post" style="margin:1.5em 0">';
...
echo '<div style="padding:5px 0;">Status: <span style="color:red;font-weight:bold;">Manual</span></div>';
echo '</form><hr></li><li class="demo"><form action="options-general.php?page=pariwager-processing" method="post" style="margin:1.5em 0;">';
echo '<input type="number" value="0" name="number-of-plays" />';
echo '<div style="height:5px;"></div>';
echo '<div style="padding:5px 0;">Enter a value:<span style="font-weight:bold;"><br> 0 for projection data only. <br>1-178 for games in progress. <br>179-250 for games are done.</span></div>';
echo '<div style="height:5px;"></div>';
echo '<input type="hidden" value="" name="create_nfl_demo_projections_and_contests_button_clicked" />';
submit_button('Create NFL Demo Projections and Contests');
echo '<div style="padding:5px 0;">Status: <span style="color:red;font-weight:bold;">Manual</span></div>';
...
jQuery(document).ready(function(){
	...
	jQuery('input[name="contest-type"]').change(function(){
        if( jQuery(this).is(':checked') && jQuery(this).val() == 'demo' ){
			jQuery('.demo').show();
			jQuery('.regular').hide();
        }else{
			jQuery('.demo').hide();
