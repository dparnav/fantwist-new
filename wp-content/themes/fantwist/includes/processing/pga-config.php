...
if ($show_updated_points_message == true) {
	echo '<div id="message" class="updated fade"><p>PGA points algorithm updated.</p></div>';
}

if (isset($_POST['create_pga_projections_and_contests_button_clicked'])) {
	...
	create_pga_projections_and_contests($date, $projection_key);

}
if (isset($_POST['update_live_pga_contests_button_clicked'])){
	
	$stats_key = '44691bd017be4bdabcd8af9da127ae38';
	
	update_live_pga_contests($stats_key, false);
	
	update_option( 'pga-last-updated-live', date('m-d-Y g:i a', time() - (60*60*7))); 

}
if (isset($_POST['process_finished_pga_contests_button'])) {
	
	$stats_key = '44691bd017be4bdabcd8af9da127ae38';
	
	process_finished_pga_contests($stats_key);
...
// PGA Dashboard table
echo '<div class="table league-table league-table-pga">' .
'<div class="table-row strong align-center">' . 
'<div class="table-cell middle table-league-name align-center"><strong>PGA</strong></div>' . 
...
echo '<input type="hidden" value="" name="update_live_pga_contests_button_clicked" />';
submit_button('Update Live PGA Contests');
...
echo '</form><hr></li><li><form action="options-general.php?page=pariwager-processing" method="post" style="margin:1.5em 0;">';
echo '<input type="hidden" value="" name="process_finished_pga_contests_button" />';
submit_button('Process Finished PGA Contests and Wagers');
echo '<div style="padding:5px 0;">Status: <span style="color:red;font-weight:bold;">Manual</span></div>';
...
settings_fields( 'pariwager-pga-points' );
do_settings_sections( 'pariwager-pga-points' );
...
'<div class="update-points-btn update-pga-points">';
