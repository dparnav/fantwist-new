									...
									foreach ($team_players as $team => $team_player) {
										...
										if (!empty($results) && $game_from_contest) {
											foreach (${$team}->player as $key => $player) {
												switch ($key) {
													case "P":
														${$team_player}['Pitchers'] += (float)$player;
														break;
													case "SP":
														${$team_player}['Pitchers'] += (float)$player;
														break;
													...
													case "1B":
														${$team_player}['Hitters'] += (float)$player;
														break;
													case "2B":
														${$team_player}['Hitters'] += (float)$player;
														break;
									...
									foreach ($team_players as $team => $team_player) {
										...
										if (!empty($results) && $game_from_contest) {
											foreach (${$team}->player as $key => $player) {
												switch ($key) {
													case "C":
														${$team_player}['C'] += (float)$player;
														break;
													case "PF":
														${$team_player}['F'] += (float)$player;
														break;
													case "SF":
														${$team_player}['F'] += (float)$player;
														break;
													case "PG":
														${$team_player}['G'] += (float)$player;
														break;
													case "SG":
														${$team_player}['G'] += (float)$player;
														break;
									...
									foreach ($team_players as $team => $team_player) {
										...
										if (!empty($results) && $game_from_contest) {
											foreach (${$team}->player as $key => $player) {
												switch ($key) {
													case "G":
														${$team_player}['Goalie'] += (float)$player;
														break;
													default:
														${$team_player}['Players'] += (float)$player;
														break;
												}
											}
										}
									...
									foreach ($team_players as $team => $team_player) {
										...
										if (!empty($results) && $game_from_contest) {
											foreach (${$team}->player as $key => $player) {
												switch ($key) {
													case "QB":
														${$team_player}['QB'] += (float)$player;
														break;
													case "RB":
														${$team_player}['RB'] += (float)$player;
														break;
													case "WR":
														${$team_player}['WR'] += (float)$player;
														break;
													case "TE":
														${$team_player}['TE'] += (float)$player;
														break;
													default:
														${$team_player}['DST'] += (float)$player;
														break;
												}
											}
										}
