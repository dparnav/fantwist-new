...
<noscript>
	<img height="1" width="1" src="https://www.facebook.com/tr?id=462961841188252&ev=PageView
&noscript=1" />
</noscript>
	...
	jQuery(document).ready(function($) {
			...
			if (gameType == 'Team vs Team') {
				...
				if (wagerAmount.match(/^\d+\.\d+$/) || wagerAmount.match(/^-{0,1}\d+$/)) {
					...
					if (wagerAmount > 0 && wagerAmount < 501) {
						...
						if (window.teamvsteamType == 'spread' || window.teamvsteamType == 'overunder') {

							winnings = parseFloat(wagerAmount) * 0.9090909090909092;
							winnings = parseFloat(winnings);

						} else if (window.teamvsteamType == 'moneyline') {

							window.moneylineVal = parseFloat(window.moneylineVal.toString().replace(',', ""));

							if (window.moneylineVal > 0) {
								winnings = (parseFloat(wagerAmount) / 100) * (Math.abs(window.moneylineVal));
								winnings = parseFloat(winnings);
							} else {
								winnings = (parseFloat(wagerAmount) / (Math.abs(window.moneylineVal)) * (100));
								winnings = parseFloat(winnings);
							}
				...
				if ($('.submit-bet').hasClass('disabled')) {
					$('.submit-bet').prop("disabled", true);
				} else {
					$('.submit-bet').prop("disabled", false);
				...
				var wagerType = $('.select-wager').val(),
					winnings, denom, wagerOdds = [],
					wagerData, teamNumber, teamPlayers,
					wagerTeamCount = $('.contest-tab.show').length,
					wagerTeamInvalid = false,
					teams = [];
		...
		$('.lobby-tabs a').click(function(e) {
			...
			if (count == 0) {
				...
				setTimeout(function() {
					$('.lobby-games').append('<div class="no-contests">There are no upcoming games. Please try after some time.</div>');
				}, 250)
		...
		$('.confirmation-popup-box-close').click(function() {
			$('#confirmationPopupBox').fadeOut();
			// $('#popupBetAmout').html('');
			$('#popupBetType').html('');
			$('#popupBetDetails').html('');
		});
		...
		$('.contest-main').on('click', '.contest-team.show-overlay', function(e) {
			...
			$('.contest-tab.show .select-team').each(function() {
				...
				if (selectedTeam == 'Team') {
					...
					$('html, body').animate({
						scrollTop: $('.contest-tabs-wrap').offset().top - 60
					}, 500);
		...
		$('.team-vs-team-bet').click(function(e) {
			...
			// if (!jQuery('.team-vs-team-bet').hasClass('active')) {
				...
				var $this = $(this),
					...
					wagerGameId = $this.data('gameid');
				wagerTermId = $this.data('term-texmoney-id');

				$('#wager-game-id').val(wagerGameId);
				$('#wager-team-id').val(wagerTermId);
				if (league == 'nfl') {
				...
				if (type == 'spread') {
					...
					$('.submit-bet-form').append('<input class="rotation" name = "rotation" value="' + spreaddatarotation + '" type="hidden">');
					...
					if (league == 'nfl') {
						...
						$('.submit-bet-form').append('<input name="rotation" value="' + spreaddatarotation + '" type="hidden">');
					...
					overunderdatarotation = $this.data('rotation');

					$('.select-team-overunder').html('<option class="option-select-team" value="' + overundertext + '" name="' + overunderdatarotation + '-' + overundertext + '">' + overunderdatarotation + ' - ' + overundertext + '</option>');
					...
					$('.submit-bet-form').append('<input class="rotation" name="rotation" value="' + overunderdatarotation + '" type="hidden">');
					...
					moneylinedatarotation = $this.data('rotation');
					...
					$('.submit-bet-form').append('<input class="wager-line-type wager-line-type-' + type + '" name="wager-line-type" value="' + type + '" type="hidden">');
					$('.submit-bet-form').append('<input class="rotation" name="rotation" value="' + moneylinedatarotation + '" type="hidden">');

					if (league == 'nfl') {

						$('.submit-bet-form').append('<input id="contestID" name="wager-contest" value="' + contestID + '" type="hidden"><input id="contestDate" name="wager-contest-date" value="' + contestDate + '" type="hidden">');
						$('.submit-bet-form').append('<input name="rotation" value="' + moneylinedatarotation + '" type="hidden">');
		...
		$('.contest-main').on('click', '.refresh.no-animate', function(e) {
			...
			$.get("/refresh-" + league + "/", function() {}).done(function() {
				$.get(document.URL, function() {}).done(function(data) {
