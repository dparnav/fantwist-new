...
add_action( 'admin_init', 'pariwagerLOLpoints' );





//Update LOL Projections and Contests

function create_lol_projections_and_contests($date, $projection_key) {
	...
	*/
	...
	if ( is_array( $tourneys ) && ! is_wp_error( $tourneys ) ) {
		...
		foreach ($tourney_data as $tournament) {
			...
			if ($tourney_start == $requested_date) {
				...
				if ( is_array( $projections ) && ! is_wp_error( $projections ) ) {
					...
					$mixed_contest = array(
						'post_status' => 'publish',
						'post_title' => 'League of Legends: Mixed ' . $date_notime,
						'post_type' => 'contest',
						'meta_input' => array (
							'contest_type' => 'Mixed',
							...
							'contest_title_without_type' => 'League of Legends: ' . $date_notime_day,
					...
					$post_exists = post_exists('League of Legends: Mixed ' . $date_notime);
...
function update_live_lol_contests($stats_key, $completed) {
							...
							foreach ($team as $team_player) {
								...
								if ($playerIndex < 4) {
									...
									if ($player_id == $team_player->PlayerID) {
										...
										$mixed_results[$teamIndex]->$playerIndex->eagles = ($player->DoubleEagles+$player->Eagles);
										$mixed_results[$teamIndex]->$playerIndex->eagles_points = (($player->DoubleEagles+$player->Eagles)*$player->TotalStrokes*$eagles_VAL);
										...
										$total_points = (($player->DoubleEagles+$player->Eagles)*$eagles_VAL) + ($player->Birdies*$birdies_VAL) + ($player->Pars*$pars_VAL) + ($player->Bogeys*$bogeys_VAL) + ($doublebogeyorworse_VAL*($player->DoubleBogeys+$player->WorseThanDoubleBogey)) + ($player->HoleInOnes*$holesinone_VAL) + $finish_points;
...
function process_finished_lol_contests($stats_key) {
...
add_action( 'lol_update_live_cron', 'lol_update_live_cron' );

function lol_update_live_cron() {
	...
	$cron_log = array(
		'post_status' => 'draft',
		'post_title' => 'League of Legends Cron Log - Update Live',
		'post_type' => 'cron_log',
