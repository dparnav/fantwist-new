...
if (is_user_logged_in()) {
	...
	if (isset($_POST['wager-type']) && isset($_POST['wager-game'])) {
		...
		$wager_amount = number_format($_POST['wager-amount'], 2);
		...
		foreach ($gamedeatils_query->posts as $game) {
			...
			if (get_field('bidding_status', $game->ID) != 0) {
				echo '
				<script>
					alert("Sorry, the game has been closed for betting! Place a bet on a different contest.");
		...
		if ($existing_wager_amount + $wager_amount > 500) {
			echo '
			<script>
				alert("Sorry, the maximum allowed is $500 for each type of bet on a single game. Please try betting a smaller amount or place a different bet.");
				window.location = "' . get_home_url() . '/contest/' . $_POST['wager-contest'] . '";
			</script>
			';
			exit;
...
//ajax function call
if (isset($_POST['get_current_scores']) && !empty($_POST['get_current_scores'])) {
	$get_current_scores = $_POST['get_current_scores'];
	$get_current_scores();
}
												...
												<div class="contest-lines-select selectdiv">
													...
													<select class="select-team select-team-moneyline" name="wager-team-vs-moneyline">
														...
														?>
																...
																<option class="option-select-team option-select-team-<?php echo $team->term_taxonomy_id; ?>" value="<?php echo $team->term_taxonomy_id; ?>_<?php echo $team->moneyline; ?>" data-gameid="<?php echo $game_id; ?>" name="<?php echo $team->name; ?>" data-spread="<?php echo $team->moneyline; ?>"><?php echo  $team->rotation_number . " - " . $team->name; ?> <?php if ($team->moneyline > 0) {
																																																																																																									echo '+';
																																																																																																								}
																																																																																																								echo $team->moneyline; ?></option>
					...
					if ($the_query->have_posts()) {
						while ($the_query->have_posts()) {
							...
							if ($nfl_main_contest == '') {
								...
								echo '<div class="contest-game-2 contest-' . $recent_league_slug . ' ' . $recent_locked . '">' .
						...
						if ($the_query2->have_posts()) {
							...
							while ($the_query2->have_posts()) {
								...
								if ($wager_type == 'Spread') {
									$wager_list_html = $wager_list_html . '<div class="wager-list-item ' . $wager_list_class . '" data-team="' . htmlspecialchars(json_encode($winners), ENT_QUOTES, 'UTF-8') . '" data-type="' . $wager_type . '">' . $data_rotation . ' - ' . $wager_team_names . ' - Bet $' . number_format($wager_amount, 2) . ' to Win $' . $wager_list_winnings . ' ' . $wager_list_result . '</div>';
								} else if ($wager_type == 'Over/Under') {
									$wager_list_html = $wager_list_html . '<div class="wager-list-item ' . $wager_list_class . '" data-team="' . htmlspecialchars(json_encode($winners), ENT_QUOTES, 'UTF-8') . '" data-type="' . $wager_type . '">' . $data_rotation . ' - ' . $wager_team_names . ' - Bet $' . number_format($wager_amount, 2) . ' to Win $' . $wager_list_winnings . ' ' . $wager_list_result . '</div>';
								} else if ($wager_type == 'Moneyline') {
									$wager_list_html = $wager_list_html . '<div class="wager-list-item ' . $wager_list_class . '" data-team="' . htmlspecialchars(json_encode($winners), ENT_QUOTES, 'UTF-8') . '" data-type="' . $wager_type . '">' . $data_rotation . ' - ' . $wager_team_names . ' - Bet $' . number_format($wager_amount, 2) . ' to Win $' . $wager_list_winnings . ' ' . $wager_list_result . '</div>';
									...
									foreach ($all_contest_data as $game) {
										...
										if ($team_1_team != "" && $team_2_team != "") {
											if ($current_game_date != $previous_game_date) {
												echo '<h2 class="section-header"> ' . $current_game_date . ' </h2>';
											...
											echo '<div class="contest-team contest-vs contest-vs-' . $game_count . '" data-gameid="' . $game_id . '">' .
												...
												'<div class="contest-teamvsteam-cell team-vs-team-bet transition contest-moneyline bord contest-status-' . str_replace(' ', '-', strtolower($contest_status_html)) . '" data-name="' . $team_1_team . '" data-spread="' . $moneyline_1_plus . $team_1_moneyline . '"data-rotation="' . $data_rotation_1 . '" data-select="moneyline" data-term-texmoney-id="' . $team_1_term_taxonomy . '" data-gameid="' . $game_id . '"><strong>' . $moneyline_1_plus . $team_1_moneyline . '</strong></div>' .
												'<div class="contest-teamvsteam-cell team-vs-team-bet transition contest-moneyline contest-status-' . str_replace(' ', '-', strtolower($contest_status_html)) . '" data-name="' . $team_2_team . '" data-spread="' . $moneyline_2_plus . $team_2_moneyline . '"data-rotation="' . $data_rotation_2 . '" data-select="moneyline" data-term-texmoney-id="' . $team_2_term_taxonomy . '" data-gameid="' . $game_id . '"><strong>' . $moneyline_2_plus . $team_2_moneyline . '</strong></div>' .
												'</div>' .
												'</div>' .
												'</div>';
									...
									foreach ($contest_data as $game) {
										...
										if ($team_1_team != "" && $team_2_team != "") {
											echo '<div class="contest-team contest-vs contest-vs-' . $game_count . '" data-gameid="' . $game_id . '">' .
												...
												'<div class="contest-teamvsteam-cell team-vs-team-bet transition contest-moneyline bord contest-status-' . str_replace(' ', '-', strtolower($contest_status_html)) . '" data-name="' . $team_1_team . '" data-select="moneyline" data-term-texmoney-id="' . $team_1_term_taxonomy . '" data-gameid="' . $game_id . '" data-spread="' . $team_1_moneyline . '"data-rotation="' . $data_rotation_1 . '"><strong>' . $moneyline_1_plus . $team_1_moneyline . '</strong></div>' .
												'<div class="contest-teamvsteam-cell team-vs-team-bet transition contest-moneyline contest-status-' . str_replace(' ', '-', strtolower($contest_status_html)) . '" data-name="' . $team_2_team . '" data-select="moneyline" data-term-texmoney-id="' . $team_2_term_taxonomy . '" data-gameid="' . $game_id . '" data-spread="' . $team_2_moneyline . '"data-rotation="' . $data_rotation_2 . '"><strong>' . $moneyline_2_plus . $team_2_moneyline . '</strong></div>' .
												'</div>' .
												'</div>' .
												'</div>';
...
?>
	...
	if ($the_query->have_posts()) {
			...
			foreach ($the_query->posts as $wager_post) {
				...
				$existing_wager_type = get_field('wager_type', $wager_post_id) == "Over/Under" ? "overunder" : get_field('wager_type', $wager_post_id);
				...
				$date = date('M-d-Y g:ia', get_field('contest_date', $wager_post_id));
				...
				?>
					...
					<input id="wagerAmount" value="<?php echo "Bet " . $wager_amount . " To win " . $potential_winning . "<br>"; ?>" type="hidden">
	...
	<script>
		jQuery(document).ready(function() {
			...
			jQuery('.confirmation-popup-box-title').html("Straight bet - Sold");
			...
			jQuery('#popupBetTeam').html('<div class="bet-title-popup">Team: </div><div class="bet-value-popup">' + contestTeam + '</div>');
				...
				if (Object.keys(newData).length !== 0) {
					...
					if (confirm("Note that the details of this contest have been updated. Would you like to continue?")) {
						...
						if (type == 'overunder') {
							...
							if (type == 'moneyline') {
								window.moneylineVal = updatedScore;
							}
